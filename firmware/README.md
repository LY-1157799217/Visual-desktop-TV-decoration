...
| 只有背光亮、纯黑 | ⚠️ **还不能断定是硬件坏了** ⇒ 按[根 README 的「🆘 先做这两件事」](../README.md)依次查：① 供电与接线 ② 库目录里的 `User_Setup.h` 到底是哪一份 ③ 串口启动日志有没有报错 |
| 连背光都不亮 | 供电/背光脚问题 |
...
> **当前保留已验证方案，合并方案的启动异常尚未定位**；完整记录见 [platformio.ini](platformio.ini) 末尾。
