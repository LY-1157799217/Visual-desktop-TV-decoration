...
```bash
pip install esptool
# 插上USB-C,设备管理器确认COM号(下面以COM7为例)
python -m esptool --chip esp32c3 --port COM7 read_flash 0x0 0x400000 stock-backup-factory.bin
```

恢复出厂:`python -m esptool --chip esp32c3 --port COM7 write_flash 0x0 stock-backup-factory.bin`
