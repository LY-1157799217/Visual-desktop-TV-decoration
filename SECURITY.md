...
| 重新烧录一版不配网的固件 | ⚠️ **清不掉凭据。** 烧录只写 app 区，NVS 原样保留（实测：跑过不配网固件前后，NVS 分区 sha256 逐字节相同） |
| 最彻底：整片 flash 擦除 | `esptool.py erase_flash` —— **唯一能同时清掉 NVS 与 SPIFFS 的办法** |

> ⚠️ **转让/换机时别只按「重置 WiFi」**：它只清 NVS，
> **SPIFFS 里你上传的相册图片、网页背景、城市数据仍然都在**。
> 而控制台**没有删除文件的功能**（代码里唯一的 `SPIFFS.remove` 是开机时清理旧版本遗留文件的），
> 想彻底清干净只能用整片擦除；否则只能用新图逐个覆盖。
